...
<div className="md:ml-60 md:p-10 p-6 space-y-8">

  {/* Cabeçalho com Título e Botão de Novo Relatório */}
  <div className="flex justify-between items-center bg-white shadow-sm rounded-lg p-6">
    ...
    <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
      <DialogTrigger asChild>
        <Button className="bg-blue-600 text-white py-3 px-6 rounded-md hover:bg-blue-700 transition-all duration-200">
  ...
  <div className="relative w-full max-w-md">
    <Search className="absolute left-4 top-1/2 transform -translate-y-1/2 w-5 h-5 text-gray-500 transition-all" />
    <Input
      ...
      <TableBody>
        {filteredReports.map((report) => (
          <TableRow key={report.id} className="hover:bg-gray-50 transition-all">
            ...
            <TableCell>{report.ordemServico?.custo_estimado.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}</TableCell>
            <TableCell>{report.custo_total.toLocaleString('pt-BR', { style: 'currency', currency: 'BRL' })}</TableCell>
            <TableCell>{new Date(report.data_criacao).toLocaleDateString('pt-BR')}</TableCell>
            <TableCell>
              <Badge 
                variant="outline" 
                className={`${
                  report.ordemServico?.status === 'completed' ? 'bg-green-100 text-green-800' : 
                  report.ordemServico?.status === 'in_progress' ? 'bg-yellow-100 text-yellow-800' : 
            ...
            <TableCell className="flex justify-end">
              <Button 
                variant="outline" 
                size="icon" 
                className="text-blue-600 hover:bg-blue-50 p-2 rounded-full transition-all mr-4"
                onClick={() => {
